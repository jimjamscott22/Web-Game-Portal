...
export default function BoardLightsOut({ difficulty, onMove, onWin, resetKey }: { difficulty: Difficulty; onMove: () => void; onWin: () => void; resetKey: number }) {
  ...
  return <><div className="grid gap-2 p-3 bg-[#20242D] border-4 border-dark rounded-2xl mx-auto" style={{ gridTemplateColumns: `repeat(${size}, 1fr)`, maxWidth: 520 }}>{grid.flatMap((row, r) => row.map((lit, c) => { const i = r * size + c; return <button key={i} onClick={() => { setCursor(i); choose(i); }} className={`lights-cell aspect-square min-h-11 rounded-lg border-[3px] border-dark ${lit ? 'is-lit bg-[#FDC846]' : 'bg-[#4E5969]'} ${cursor === i ? 'ring-4 ring-white ring-offset-2 ring-offset-dark' : ''}`} aria-label={`Light ${r + 1}, ${c + 1}, ${lit ? 'on' : 'off'}`} aria-pressed={lit} />; }))}</div><MobileControls color="#FDC846" onUp={() => move(-size)} onDown={() => move(size)} onLeft={() => move(-1)} onRight={() => move(1)} /></>;
}
